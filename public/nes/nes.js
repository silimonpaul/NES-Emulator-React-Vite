function Nes() {
  ...
  this.loadRom = function (rom) {
    if (rom.length < 0x10) {
      log("Invalid rom loaded - ROM too small");
      return false;
    }
    if (
      rom[0] !== 0x4e ||
      ...
      log("Invalid rom loaded - Not a valid iNES format");
      return false;
    }
    let header = this.parseHeader(rom);
    if (rom.length < header.chrBase + 0x2000 * header.chrBanks) {
      log(
        "Rom file is missing data - Expected size: " +
          (header.chrBase + 0x2000 * header.chrBanks) +
          ", Actual size: " +
          rom.length
      );
      return false;
    ...
    if (mappers[header.mapper] === undefined) {
      log(
        "Unsupported mapper: " +
          header.mapper +
          " - This ROM uses a mapper that isn't implemented yet"
      );
      return false;
    } else {
      try {
    ...
    // Add validation for successful mapper initialization
    if (!this.mapper || !this.mapper.read || !this.mapper.write) {
      log("Mapper initialization failed - Critical mapper functions missing");
      return false;
    }
    ...
    log(
      "Loaded " +
        ...
        header.mapper +
        "\nMirroring: " +
        (header.verticalMirroring ? "Vertical" : "Horizontal") +
        "\nBattery: " +
        (header.battery ? "Yes" : "No")
    );
    return true;
  };
  ...
  this.setState = function (obj) {
    if (
      obj.version !== this.stateVersion ||
      obj.mapperVersion !== this.mapper.version
    ) {
      return false;
    }
    // check header
